...
const update = async (req, res) => {
    try {
        ...
        res.status(200).send({ menssagem: `Encontro de id ${encontro.id} foi atualizado com sucesso` }).end()
    } catch (error) {
        res.status(200).send({ menssagem: `Erro ${error.code}, id do encontro não foi achado` }).end()
    }
...
const del = async (req, res) => {
    try {
        ...
        res.status(200).send({ menssagem: `Encontro de id ${encontro.id} foi excluido com sucesso` }).end()
    } catch (err) {
        res.status(200).send({ menssagem: "erro" }).end()
    }
}
